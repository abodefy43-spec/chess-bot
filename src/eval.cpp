...
// PeSTO-style tapered evaluation (Ronald Friederich / chess programming wiki).
// Tables are written from white's perspective with index 0 = a8 ("reading order"
// from rank 8 down to rank 1). White piece on square s uses table[s ^ 56];
// black piece on square s uses table[s] and we negate.

static const int mg_value[7] = { 0, 82, 337, 365, 477, 1025, 0 };
static const int eg_value[7] = { 0, 94, 281, 297, 512, 936,  0 };

// Game-phase increments per piece type (N,B,R,Q)
...
int evaluate(const Position& pos) {
    ...
    for (int pt = PAWN; pt <= KING; ++pt) {
        const int* mt = mg_table[pt];
        const int* et = eg_table[pt];
        int mv = mg_value[pt];
        int ev = eg_value[pt];

        Bitboard wbb = pos.pieces(WHITE, PieceType(pt));
        Bitboard bbb = pos.pieces(BLACK, PieceType(pt));
        while (wbb) {
            Square s = BB::pop_lsb(wbb);
            mg[WHITE] += mv + mt[s ^ 56];
            eg[WHITE] += ev + et[s ^ 56];
            phase += phase_inc[pt];
        }
        while (bbb) {
            Square s = BB::pop_lsb(bbb);
            mg[BLACK] += mv + mt[s];
            eg[BLACK] += ev + et[s];
            phase += phase_inc[pt];
        }
    ...
    // Bishop pair bonus
    if (BB::popcount(pos.pieces(WHITE, BISHOP)) >= 2) score += 30;
    if (BB::popcount(pos.pieces(BLACK, BISHOP)) >= 2) score -= 30;
